...
const LoanApp = React.forwardRef((props, ref) => {
	...
	//         ReactGA.event({
	//             category: 'Apply Now Button | DigitalCrafts',
	//             action: 'click',
				...
				<form className={formName} onSubmit={handleSubmit}>
					...
					{multiplePrograms && (
						<div className="w-full lg:w-1/2 px-8 lg:px-0">
							...
							<p
								className={active.program0 ? activeClass : inactiveClass}
								onClick={() => toggleIsActive(0)}
							>
								{programNameAndURL[0].name}
							</p>
							<p
								className={active.program1 ? activeClass : inactiveClass}
								onClick={() => toggleIsActive(1)}
							>
								{programNameAndURL[1].name}
							</p>
						</div>
					)}
					<div className="hidden">
						<input type="text" name="Stakeholder Type" value="Student" readOnly />
						<input type="text" name="Program Name" value={programNameAndURL.programName} readOnly />
